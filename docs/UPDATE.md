...
v0.0.1 には自動アップデート機構が同梱されていないため、v0.0.2 への移行のみ手順が異なります。
[`UPGRADE-0.0.1-to-0.0.2.md`](UPGRADE-0.0.1-to-0.0.2.md) を参照してください（AI に渡すプロンプト付き）。
...
3. ファイルが存在しない（利用者が削除）→ **復活させない**。lock には `"deleted"` として記録され、以後の更新・配布未完了チェックでも削除の意図が尊重されます
